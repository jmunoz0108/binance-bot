"""
WARFARE FILTER — Final version with built-in Flask API
"""
...
log.info("=" * 55)
log.info("WARFARE FILTER — Starting")
...
class MTF:
    ...
    def bias(self,sym):
        now=time.time()
        if sym in self._cache and now-self._ts.get(sym,0)<900: return self._cache[sym]
        try:
            kl=self.c.get_klines(symbol=sym,interval='4h',limit=60)
            cl=[float(k[4]) for k in kl]
            if len(cl)<20: return 'NEUTRAL'
            e20=_ema(cl,20); e50=_ema(cl,min(50,len(cl))); rv=_rsi(cl); pr=cl[-1]
            if pr>e20>e50 and rv>52: b='BULL'
            elif pr<e20<e50 and rv<48: b='BEAR'
            else: b='NEUTRAL'
            self._cache[sym]=b; self._ts[sym]=now; return b
...
class Dedup:
    ...
    def ok(self,sym,action):
        ...
        if (datetime.now()-self._s[sym]['ts']).seconds>3600: del self._s[sym]; return True
        return False
...
def score_opp(opp, scanner_data, mtf, regime):
    sym=opp.get('symbol',''); action=opp.get('action',''); strat=opp.get('strategy','')
    base=opp.get('score',50); score=base; bd={}
    # Regime
    if regime=='BULL' and action=='LONG': score+=10; bd['regime']='+10'
    elif regime=='BEAR' and action=='SHORT': score+=10; bd['regime']='+10'
    elif regime=='BULL' and action=='SHORT': score-=20; bd['regime']='-20'
    elif regime=='BEAR' and action=='LONG':
        score-=15; bd['regime']='-15'
        if strat!='Volume Spike Reversal': score-=5
    else: bd['regime']='0'
    ...
    if action not in ('FUNDING_SHORT',):
        ...
        elif b4!='NEUTRAL': score-=12; bd['mtf']=f'-12({b4})'
        else: bd['mtf']='0'
    # OB
    ...
    if action=='LONG':
        ...
        elif ob<-0.15: score-=12; bd['ob']='-12'
        else: bd['ob']='0'
    elif action=='SHORT':
        if ob<-0.25: score+=15; bd['ob']='+15'
        elif ob<-0.10: score+=8; bd['ob']='+8'
    ...
    # Buy pressure
    bp=ta.get('buy_pressure',0.5)
    if action=='LONG':
        if bp>=0.65: score+=10; bd['tape']='+10'
        elif bp<=0.35: score-=8; bd['tape']='-8'
        else: bd['tape']='0'
    elif action=='SHORT':
        if bp<=0.35: score+=10; bd['tape']='+10'
        elif bp>=0.65: score-=8; bd['tape']='-8'
        else: bd['tape']='0'
    # FG
    fg=scanner_data.get('fear_greed',50)
    if fg<20 and action=='SHORT': score-=10; bd['fg']='-10'
    elif fg>80 and action=='LONG': score-=10; bd['fg']='-10'
    else: bd['fg']='0'
...
class Filter:
    def __init__(self):
        ...
        self.mtf=MTF(self.client); self.dedup=Dedup()
    ...
    def _fetch_scanner(self):
        ...
        except Exception as e: log.warning(f"Scanner fetch: {e}")
        return {}
    ...
    def run_once(self):
        ...
        if not opps:
            ...
            STATE={'timestamp':datetime.now().isoformat(),'signals':[],
                   'total_passed':0,'total_evaluated':0,'regime':data.get('regime','NEUTRAL'),
